...
class OptimalSupply:
    ...
    Attributes:
        ...
        predicted_metrics: PerformanceMetrics at optimal headcount
...
class SupplyOptimizer:
    ...
    Uses a digital twin (PerformanceEmulator) to evaluate staffing levels
    ...
    def __init__(
        ...
        """Initialize optimizer with a performance emulator.
        
        Args:
            emulator: PerformanceEmulator instance for predicting metrics.
    ...
    def _meets_constraints(
        ...
        """
        return (
            metrics.sla_compliance >= constraints.min_sla
            and metrics.wait_time <= constraints.max_wait_time
            and metrics.occupancy <= constraints.max_occupancy
        )
    ...
    def optimize(
        ...
        if demand <= 0:
            metrics = self.emulator.predict_metrics(0, 0)
        ...
        for supply in range(1, self.max_supply + 1):
            metrics = self.emulator.predict_metrics(demand, supply)
        ...
        last_metrics = self.emulator.predict_metrics(demand, self.max_supply)
        return OptimalSupply(
