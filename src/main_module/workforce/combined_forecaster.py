...
class CombinedForecaster:
    ...
    def _smooth_anomalies(self, df):
        for start, end in _KNOWN_ANOMALIES:
            ...
            if mask.any():
                ...
                df["call_count"] = (
                    df["call_count"].interpolate(method="linear").fillna(0).astype(np.int32)
                )
    ...
    def _create_base_features(self, df):
        ...
        df["is_post_tax_drop"] = ((dtd < 0) & (dtd > -365 + 335)).astype(np.int8)
    ...
    def _add_yoy_features(self, df, train_only_df):
        ...
        yoy_week = (
            train_only_df.groupby("week_of_year")["call_count"].mean().rename("yoy_same_week_mean")
        )
    ...
    def _compute_historical_patterns(self, df):
        ...
        self._hist_index = df.groupby(["month", "day_of_week", "hour", "minute"])["call_count"].agg(
            ["mean", "std", "max", "min"]
        )
    ...
    def train(self, data_path, train_year=2024, test_year=2025):
        ...
        min_d = interval_df["interval_start"].min().date()
        max_d = interval_df["interval_start"].max().date()
        print(f"  Intervals: {len(interval_df):,}  ({min_d} to {max_d})")
        ...
        last_day = df.loc[
            (df["year"] == test_year) & (df["month"] == last_complete_test_month),
            "day_of_month",
        ].max()
        if last_day < 28:
        ...
        print(f"\n  Train: {train_year} ({train_n:,} intervals)")
        print(f"  Test: {test_year} ({test_n:,} intervals)")
        ...
        lt_models = [
            LGBMRegressor(**cfg, verbosity=-1, n_jobs=-1) for cfg in self._LT_ENSEMBLE_CONFIGS
        ]
